...
For `bundle-mode=pr` (the default), `base-ref`, a full 40-character lowercase
base commit, and `head-revision` are required. Until the Git-aware PR layer is
...
Draft replacement is explicit. The new generation preserves all prior decision
receipts and adds the ready plan, answers, and phase artifacts. A directory
...
unrecorded file fails bundle validation.
