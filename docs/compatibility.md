...
| Columns add/drop/null/default | Yes | **Plannable** or **SQL handoff** | Drops require approval. A new required column without a default is added nullable; contract can assert clean rows, run reviewed cleanup before the exact gate, or stay loose in a split plan. New columns append physically; a different declarative position is reported as compatibility evidence rather than blocked. |
| Column comments | No verified diff family | **Plannable** | |
| Same-type column rename | Drop/create-oriented diff | **Decision required**, then **Plannable**, **SQL handoff**, or split | Eligible columns use a generated two-column, two-way bridge. A retained dependency that PostgreSQL can prove is preserved automatically. An intentional or unproved dependent-view change gets three ordered pockets: expand overlap, contract removal, and exact desired recreation. |
| Same-name column type change | Yes | **SQL handoff** or split | Two edit pockets own product conversion semantics. For catalog-proven dependents, onwardpg orders generated reverse drops and desired recreation around contract. It never substitutes a direct `ALTER COLUMN TYPE` for a rolling-deployment plan. |
| Cross-name/type column transition | Drop/create-oriented diff | **Decision required**, then **SQL handoff** or split | Cross-type identity requires an explicit rename hint. The accepted transition owns both columns and the current/desired dependency closure; exactly two pockets own the whole expand and contract implementation, so no independent endpoint or view DDL can escape. |
...
| Ordinary view create/replace/drop | Yes | **Plannable** for PostgreSQL-legal replacements | Typed dependencies on relations, columns, enums, views, and modeled routines. `CREATE OR REPLACE VIEW` is emitted only when existing output names, order, and types stay fixed; compatible appended outputs are allowed. |
| Ordinary view inside a column transition | No rollout model | **Plannable** or **SQL handoff** | A catalog-provable retained shape can stay generated. Otherwise the owning transition names the closure and supplies ordered overlap/removal/recreation pockets instead of attempting an illegal output rename. |
...
| Materialized view inside a column transition | No rollout model | **SQL handoff** or split | The owning transition includes the materialized view and indexes in its closure. The developer or agent chooses rebuild/refresh semantics and proves freshness; onwardpg does not claim automatic availability or data-currentness. |
