...
-- Review: safety=review; hazards=final_overlap_catchup,data_movement,table_scan,unbounded_update,long_transaction,wal_volume,replica_lag.
UPDATE "app"."accounts" SET "full_name" = "display_name" WHERE "full_name" IS DISTINCT FROM "display_name";
-- Review: safety=review; hazards=rename_equality_assertion,table_scan.
...
-- Review: safety=review; hazards=compatibility_removal,table_lock.
DROP TRIGGER "onwardpg_sync_column_4cff936be08db67c" ON "app"."accounts";
-- Review: safety=review; hazards=compatibility_removal.
DROP FUNCTION "app"."onwardpg_sync_column_4cff936be08db67c"();
-- Review: safety=dangerous; hazards=compatibility_removal,data_loss,access_exclusive_lock.
ALTER TABLE "app"."accounts" DROP COLUMN "full_name";
-- Review: safety=review; hazards=compatibility_removal,access_exclusive_lock.
