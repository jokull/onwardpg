...
| Same-type column rename | Expand shadow column, trigger, initial backfill; contract catch-up and native rename | Both names remain writable; divergent dual writes fail; pre-deployment writers drain before bridge removal | `TestBuildRequiresFingerprintBoundColumnRenameAnswer`, `TestColumnRenameOverlapBridgeConvergesOnPostgreSQL`, `TestColumnRenameWithExistingTriggerIsRejectedOnPostgreSQL` |
