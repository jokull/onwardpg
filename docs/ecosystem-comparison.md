...
| Ownership / grants / RLS | Typed table ownership in live graphs, table grants, and RLS/policies with intent questions; restricted declarative materialization cannot assume an external owner role; default/column/non-table ACLs, non-table ownership, and grant chains block | Optional privileges and RLS/policies; sequence ownership | Table ownership on current `main`; other security blocked | Table privileges and RLS/policies; ownership generally absent |
