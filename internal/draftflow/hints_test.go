...
func TestBuildPlanConsumesAheadOfTimeRenameHint(t *testing.T) {
	...
	if plan.Status != protocol.NeedsInput || len(plan.Questions) != 1 || plan.Questions[0].Kind != "rename_backfill_strategy" {
...
func TestBuildPlanAcceptsResendingAnAlreadyReceiptedHint(t *testing.T) {
	...
	if plan.Status != protocol.NeedsInput || len(plan.Questions) != 1 || plan.Questions[0].Kind != "rename_backfill_strategy" || answers == nil || len(answers.Answers) != 1 || len(hints) != 1 {
