...
_Reviewed 21 July 2026 against the primary documentation linked on this page._
...
The boundary is explicit. A simple catalog-proven rename keeps its dependents
on the generated path. If the same-type rename intentionally changes a view,
onwardpg keeps the generated column bridge and adds three bounded pockets for
the overlap, pre-cutover removal, and final recreation. A confirmed rename plus
type change gets two larger pockets—expand and contract—which own both columns
and the entire current/desired dependency closure. That path needs more help,
but it no longer leaks an unrelated column drop or illegal view replacement
beside the handoff.
