...
onwardpg knows which schemas are playing which role. The durable migration is replayed history → working intent. A separate development reconciliation handles a branch-worn local database without contaminating the production bundle. Production appears only in explicit read-only operations: drift against accepted history, contract readiness against the verified post-expand checkpoint, or bounded evidence gathered by an authorized agent. Verification uses independent disposable replays.
